...
void data()
{
  if (FreqMultRes.available())
  {
    resSum += FreqMultRes.read();
    resCount++;
  }
  if (FreqMultCap.available())
  {
    capSum += FreqMultCap.read();
    capCount++;
  }
...
void printData()
{
  ...
  freqCap = (capCount > 0) ? FreqMultCap.countToFrequency(capSum / capCount) : -1;

  if (Serial)
  {
    sprintf(pidStr, "Motor Set: %d Motor Pos: %ld PID Err: %.2f PID Val %d PID P: %.2f PID I: %.2f PID D: %.2f", motorSetPosition, motor.currentPosition(), PIDerror, PIDvalue, PIDp, PIDi, PIDd);
    sprintf(tempSetStr, "Set Temp (C°): %3.2f Set Temp Count : %d ", setTemperature, setTemperatureCounter);
    sprintf(tempStr, "Room Temp (C°): %3.2f Wash Temp (C°): %3.2f Outlet Temp (C°): %3.2f Tower Temp (C°): %3.2f", tempRoom, tempWash, tempOutlet, tempTower);
    sprintf(massStr, "M: %3.2f ΔM: %3.2f Checkpoint: %d", mass, massDerivative, checkpoint);
    sprintf(freqStr, "Frequency (RES): %ld Frequency (CAP): %ld\n", freqRes, freqCap);
...
void temperatureIncrementer()
{
  ...
  {
    checkpoint = tempTime + checkpointIncrement;
  ...
  elapsedTime3 = (tempTime - previousTime3);
  if (elapsedTime3 >= 1)
  {
  ...
  if (setTemperatureCounter == setTemepratureCounterMax && tempTime >= checkpoint)
  {
    setTemperature += 1;
    setTemperatureCounter = 0;
    checkpoint = tempTime + checkpointIncrement; // Increment for checkpoint
  }
}
